...
def list_tasks(request):
    task_groups = TaskGroup.objects.prefetch_related('task_titles__tasks').all()
    context = {
...
def send_tasks(request):
    if request.method == 'POST':
        try:
            ...
            logger.debug(f"Received data: {data}")  # Adatok nyomtatása a szerver logokba
            for task_data in data:
                task_data['created_by'] = request.user.username  # Bejelentkezett felhasználó hozzáadása
                serializer = TaskSerializer(data=task_data)
                if serializer.is_valid():
                    serializer.save()
                    logger.debug(f"Saved task: {task_data}")
                else:
